...
.PHONY: all clean deps k-deps tangle-deps ocaml-deps plugin-deps build build-ocaml build-java build-node build-bug-checker defn split-tests \
...
build: build-ocaml build-java build-node build-bug-checker
...
defn_files:=$(ocaml_files) $(java_files) $(node_files) $(bug_checker_files)
...
tests/interactive/gas-analysis/%.evm.test: build-ocaml build-java
	MODE=GASANALYZE $(TEST) $(dir $@)$*.evm $(dir $@)$*.evm.out
